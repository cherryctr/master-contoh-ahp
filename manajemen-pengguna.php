...
<table class="table table-striped table-bordered table-sm">
    ...
    <?php $no=1; foreach (data_pengguna() as $x) {
        echo "<tr>";
        echo "<td class=\"text-center\">$no</td><td>{$x['username']}</td><td class=\"text-center\">{$x['keterangan']}</td><td>{$x['nama']}</td><td class=\"text-center\"><button onclick=\"location.href='./edit-pengguna.php?username={$x[0]}'\" class=\"btn btn-primary\"><span class=\"fas fa-pen\"></span> Edit</button> <button onclick=\"hapus_pengguna('{$x[0]}')\" class=\"btn btn-danger\"><span class=\"fas fa-trash-alt\"></span> Hapus</button></td>";
        echo '</tr>';
        $no++;
    } ?>
</table>
