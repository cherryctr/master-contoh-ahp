...
function data_alternatif_periode($periode) {
    global $conn;
    $q = $conn->prepare("SELECT * FROM nilai_alternatif a join alternatif b on b.id = a.alternatif WHERE a.periode='$periode' GROUP BY alternatif");
    $q->execute();
    @$data = $q->fetchAll();
    if ($data) return $data;
    else return array();
...
function data_hasil_periode($periode) {
    global $conn;
    $q = $conn->prepare("SELECT * FROM hasil JOIN alternatif ON hasil.id_alternatif=alternatif.id WHERE hasil.periode='$periode' ORDER BY nilai DESC");
    $q->execute();
    return @$q->fetchAll();
}
...
function data_keputusan_darisampai($dari_periode,$sampai_periode) {
    global $conn;
    $q = $conn->prepare("SELECT * FROM hasil JOIN alternatif ON hasil.id_alternatif=alternatif.id WHERE hasil.periode >= '$dari_periode' AND hasil.periode <= '$sampai_periode' AND status='1' ORDER BY hasil.periode ASC");
    $q->execute();
    return @$q->fetchAll();
}
...
function data_keputusan_id($id, $periode) {
    global $conn;
    $q = $conn->prepare("SELECT * FROM hasil JOIN alternatif ON hasil.id_alternatif=alternatif.id WHERE hasil.id_alternatif='$id' AND hasil.periode='$periode'");
    $q->execute();
    return @$q->fetch();
}
